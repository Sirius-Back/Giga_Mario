"""Continue unif run21 pangenome split → Caduceus direct + adversarial.

Source (read-only):
  ``runs_unif/legnet/run21_legnet_pangenome_k10_wm100_100``
  (pangenome k=10, window −100…100, modularity-refined CCs, ≈3:1:1)

New (write):
  ``src/runs_unif/run22_caduceus_pangenome_k10_wm100_100``
  ``runs_unif/caduceus/run22_caduceus_pangenome_k10_wm100_100``

Flow:
  1. Reuse run21 ``split.csv`` (+ assignment / graph sidecars). Verify ≈3:1:1
...
SOURCE_UNIF = ROOT / "runs_unif" / "legnet" / "run21_legnet_pangenome_k10_wm100_100"
...
MAX_FOLD_SIZE = 1000  # modularity cap from run21
...
def stage_split(*, seed: int = SEED, max_fold_size: int = MAX_FOLD_SIZE) -> dict:
    """Reuse run21 split table + materialize Caduceus SPLIT (CPU only)."""
    ...
    # Ensure split.csv columns normalized; keep run21 labels (already ≈3:1:1).
    ...
    rewrite_info = {
        "method": "reuse_run21_unif_split_csv",
        ...
        "ratios": list(RATIOS),
        "note": (
            "Run21 already applied C++ contingency k=10 + modularity refine "
