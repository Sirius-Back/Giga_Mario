"""Stage validated MMseqs2 split → materialize SPLIT → LegNet direct (+ mice ZSV).

Source split (read-only):
  ``output/mmseqs_split`` — already Locked train:test:val = 60:20:20

New (write):
  ``src/runs_unif/run22_legnet_mmseqs_id08``
  ``runs_unif/legnet/run22_legnet_mmseqs_id08``

Flow:
  1. Copy ``split.csv`` + MMseqs sidecars (no ratio rewrite — already exact).
  2. Materialize SPLIT + LegNet TSV from ``ready_legnet`` PARSED/PREDICT.
  3. Wait for 1 free GPU; direct train (min 15 / max 30 / patience 10) + mice ZSV.
  4. No adversarial.

Launch::

  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run22_legnet_mmseqs_id08.continue_from_split
"""
...
SOURCE_SPLIT = ROOT / "output" / "mmseqs_split"
PANEL_ROOT = ROOT / "ready_legnet"
OUT_ROOT = ROOT / "runs_unif" / MODEL / RUN_NAME

SEED = 42
RATIOS = (0.6, 0.2, 0.2)  # train:test:val Locked
MIN_SEQ_ID = 0.8
EPOCHS = 30
MIN_EPOCHS = 15
...
BATCH_SIZE = 8192
NUM_WORKERS = 8
PREFERRED_GPUS = (2, 3, 0, 1)  # prefer free devices first
MEM_FREE_MIB = 1500
POLL_SEC = 60
PEAK_RAM_GIB_TRAIN = 24.0
PEAK_RAM_GIB_SPLIT = 24.0

MMSEQS_INTERMEDIATES = (
    "split.csv",
    "feature_table.csv",
    "sbs_assignment.csv",
...
def stage_split() -> dict:
    """Copy validated MMseqs split artifacts + materialize SPLIT + LegNet TSV."""
    ...
    _require(SOURCE_SPLIT / "split.csv", "file")
    _require(SOURCE_SPLIT / "mmseqs_split_meta.json", "file")
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_SPLIT,
        resources="stage output/mmseqs_split + materialize SPLIT + LegNet TSV",
        log=str(ROOT / "logs" / f"{RUN_NAME}_split.log"),
    )

    staged: dict[str, str] = {}
    for rel in MMSEQS_INTERMEDIATES:
        src = SOURCE_SPLIT / rel
        if not src.is_file():
            raise FileNotFoundError(f"missing MMseqs artifact: {src}")
        dest = OUT_ROOT / rel
        shutil.copy2(src, dest)
        staged[rel] = str(dest)
        print(f"staged {rel}", flush=True)

    # Optional cluster TSV for audit (large-ish but useful).
    clu = SOURCE_SPLIT / "mmseqs_work" / "clu_cluster.tsv"
    if clu.is_file():
        dest_dir = OUT_ROOT / "mmseqs_work"
        dest_dir.mkdir(parents=True, exist_ok=True)
        dest = dest_dir / "clu_cluster.tsv"
        shutil.copy2(clu, dest)
        staged["mmseqs_work/clu_cluster.tsv"] = str(dest)
    ...
    src_meta = json.loads(
        (SOURCE_SPLIT / "mmseqs_split_meta.json").read_text(encoding="utf-8")
    )
    manifest = {
        "rerun": False,
        "aligned_run": RUN_I,
        ...
        "source_meta": {
            "n_ids": src_meta.get("n_ids"),
            "n_clusters": src_meta.get("n_clusters"),
            "seed": src_meta.get("seed"),
            "min_seq_id": src_meta.get("min_seq_id"),
        },
        "staged": staged,
        "zsv": "mice",
        "adversarial": False,
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"{RUN_NAME}: source={SOURCE_SPLIT} → out={OUT_ROOT} "
