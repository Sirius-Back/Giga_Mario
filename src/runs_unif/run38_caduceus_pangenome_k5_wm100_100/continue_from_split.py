"""Continue unif run37 pangenome split → Caduceus direct + adversarial.

Source (read-only):
  ``runs_unif/legnet/run37_legnet_pangenome_k5_wm100_100``
  (pangenome k=5, reuse MARKED −100…100, modularity-refined CCs, ≈3:1:1)

New (write):
  ``src/runs_unif/run38_caduceus_pangenome_k5_wm100_100``
  ``runs_unif/caduceus/run38_caduceus_pangenome_k5_wm100_100``

Flow:
  1. Wait until run37 ``split_done.json`` + ``split.csv`` (+ graph sidecars) exist.
  2. Reuse that split table; verify ≈3:1:1 and fold sizes (do **not** re-run pangenome).
...
SOURCE_UNIF = ROOT / "runs_unif" / "legnet" / "run37_legnet_pangenome_k5_wm100_100"
...
def wait_for_source_split(*, poll_sec: int = SOURCE_POLL_SEC) -> Path:
    """Block until run37 has a reusable split (require split_done.json)."""
    ...
    print(
        ...
        f"(need split_done.json + split.csv + sbs_assignment.csv); "
    ...
    while True:
        ...
        if has_csv and has_sbs and has_done:
...
def stage_split(*, seed: int = SEED, max_fold_size: int = MAX_FOLD_SIZE) -> dict:
    """Reuse run37 split table + materialize Caduceus SPLIT (CPU only)."""
    ...
    # Ensure split.csv columns normalized; keep run37 labels (already ≈3:1:1).
    ...
    rewrite_info = {
        "method": "reuse_run37_unif_split_csv",
        ...
        "ratios": list(RATIOS),
        "note": (
            "Run37 applied pangenome k=5 on panel MARKED (−100..100) + modularity refine for large "
            "connected components; folds reused for Caduceus panel "
            "(ready_caduceus PARSED/PREDICT rematerialized). Graph viz copied from run37."
        ),
    }
