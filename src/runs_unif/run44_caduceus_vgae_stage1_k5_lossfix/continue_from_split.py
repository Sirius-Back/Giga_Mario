"""Run44: reuse run43 VGAE Stage-1 lossfix split → Caduceus direct + adversarial.

Source (read-only):
  ``runs_unif/legnet/run43_legnet_vgae_stage1_k5_lossfix``
  (VGAE stage1_region_k5_lossfix + mice ZSV overlay)

New (write):
  ``src/runs_unif/run44_caduceus_vgae_stage1_k5_lossfix``
  ``runs_unif/caduceus/run44_caduceus_vgae_stage1_k5_lossfix``

Flow:
  1. Wait until run43 ``split_done.json`` + ``split.csv`` exist.
  ...
  5. Adversarial: random ⊆ direct IDs + fold-class; max10/p5 + ZSV.
  6. Reports / TensorBoard via pipeline viz + train_monitor.

Launch::

  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs_unif.run44_caduceus_vgae_stage1_k5_lossfix.continue_from_split
"""
...
MODEL = "caduceus"
SPLIT = "vgae"
SPLIT_PARAMS = "stage1_k5_lossfix"
RUN_NAME = f"run{RUN_I}_{MODEL}_{SPLIT}_{SPLIT_PARAMS}"

SOURCE_UNIF = ROOT / "runs_unif" / "legnet" / "run43_legnet_vgae_stage1_k5_lossfix"
...
def wait_for_source_split(*, poll_sec: int = SOURCE_POLL_SEC) -> Path:
    """Block until run43 has a reusable VGAE lossfix split."""
...
def stage_split(*, seed: int = SEED) -> dict:
    """Reuse run43 VGAE lossfix split table + materialize Caduceus SPLIT (CPU only)."""
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_SPLIT,
        resources="reuse run43 VGAE stage1_k5_lossfix split + materialize ready_caduceus",
    ...
    rewrite_info = {
        "method": "reuse_run43_unif_vgae_stage1_k5_lossfix_split_csv",
        ...
        "ratios": list(RATIOS),
        "note": (
            "Run43 reused VGAE/stage1_region_k5_lossfix labeling + mice ZSV overlay; "
    ...
    manifest = {
        ...
        "gcn_model": "stage1_region_k5_lossfix",
        "ratios": list(RATIOS),
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        ...
        f"panel={PANEL_ROOT} split_only={cfg['split_only']} adversarial=true",
        flush=True,
    )
