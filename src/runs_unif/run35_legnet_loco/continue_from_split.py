"""Run35: LOCO chromosome-grain split → LegNet direct + adversarial.

New (write):
  ``src/runs_unif/run35_legnet_loco``
  ``runs_unif/legnet/run35_legnet_loco``

Flow:
  1. ``split_predict type=loco`` from ``ready_legnet`` ID.csv + mice ZSV fold.csv
     (Caduceus-aligned ratios; chromosome grain + chrom-number strata).
  2. Materialize ``SPLIT/`` + LegNet TSV (CPU; no GPU wait) — reusable via
     ``split_done.json``.
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run35_legnet_loco.continue_from_split
"""
...
SEED = 42
# Caduceus-aligned (~81/9/10) for LOCO fold→train/val/test
RATIOS = None
EPOCHS = 30
MIN_EPOCHS = 15
...
def _assert_not_random_split(split_csv: Path) -> None:
    """Sanity: LOCO folds must look like genome|chr, not random fold ids."""
    from src.pipeline.common import read_csv

    n_checked = 0
    n_loco = 0
    for row in read_csv(split_csv):
        if row["train_test"] == "zsv":
            continue
        fold = str(row["fold"])
        n_checked += 1
        if "|" in fold:
        ...
        if n_checked >= 500:
            break
    if n_checked == 0 or n_loco < max(1, n_checked // 2):
        raise RuntimeError(
            f"split.csv does not look like LOCO (genome|chr folds); "
            f"loco-like={n_loco}/{n_checked} in sample — refusing train"
        )


def stage_split(*, seed: int = SEED) -> dict:
    """Run LOCO split-predict + materialize SPLIT + LegNet TSV (CPU)."""
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_SPLIT,
        resources="loco split_predict + materialize SPLIT + LegNet TSV",
        log=str(ROOT / "logs" / f"{RUN_NAME}_split.log"),
    )

    print(f"split_predict type=loco seed={seed} ratios=Caduceus-aligned", flush=True)
    ...
    counts = _count_split(Path(split_csv))
    print(f"loco counts: {counts}", flush=True)
    _assert_not_random_split(Path(split_csv))
    ...
    manifest = {
        ...
        "split_counts": counts,
        "ratios_note": (
            "Direct: LOCO (genome|chr grain; chrom-number strata; "
            "Caduceus-aligned ratios). Adversarial random uses 3:1:1."
        ),
        "zsv": "mice",
        "direct": {
            "epochs": EPOCHS,
    ...
    return {"split_csv": Path(split_csv), "tsv": tsv, "manifest": manifest}
...
def run_train_stages(
    *,
    ...
    _assert_not_random_split(split_csv)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_done.is_file():
        ...
        _require(OUT_ROOT / "SPLIT", "dir")
        _assert_not_random_split(OUT_ROOT / "split.csv")
