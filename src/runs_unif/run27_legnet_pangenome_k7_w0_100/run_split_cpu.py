"""CPU stages for run27: reuse run17 MARKED_pangenome + new k=7 C++ graph.

- Reuse adapt from ``runs/run17_pangenome_CDS_legnet/MARKED_pangenome`` (window 0..100)
  (user "run 16 adapt" → only matching CDS 0..100 MARKED is run17; run16 is hashfrag)
- Build **new** contingency/hash graph with k=7 (cpp), modularity-refine large CCs
- Plot contingency graph colored by cluster; ratios train:test:val ≈ 3:1:1
- Materialize SPLIT + LegNet TSV under ``runs_unif/legnet/run27_legnet_pangenome_k7_w0_100``

Launch::

  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run27_legnet_pangenome_k7_w0_100.run_split_cpu
"""
...
LEGACY_MARKED = ROOT / "runs" / "run17_pangenome_CDS_legnet" / "MARKED_pangenome"
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.runs_unif.run27_legnet_pangenome_k7_w0_100.ensure_mice_fold import (
        main as ensure_fold,
    )
    ...
    meta = {
        ...
        "marked_pangenome": str(LEGACY_MARKED),
        "reuse_adapt_from": "runs/run17_pangenome_CDS_legnet/MARKED_pangenome",
        "note": "CDS window 0..100 MARKED from run17 (run16 is hashfrag, not this adapt)",
    }
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB,
        resources="reuse run17 MARKED; new C++ contingency k=7 + modularity + plot",
    ...
    split_csv = run_split_predict(
        ...
        marked_pangenome=LEGACY_MARKED,
        environment=ENVIRONMENT,
        window=WINDOW,
        kmer_size=kmer_size,
        engine=ENGINE,
