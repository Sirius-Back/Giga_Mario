"""Continue run27: 1-GPU LegNet direct + adversarial (after split_done).

Expects ``runs_unif/legnet/run27_legnet_pangenome_k7_w0_100/{split.csv,SPLIT,legnet_input}``.
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run27_legnet_pangenome_k7_w0_100.continue_from_split
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_done.is_file():
        from src.runs_unif.run27_legnet_pangenome_k7_w0_100.run_split_cpu import (
            main as split_main,
        )
