...
HOMOLOGY_EDGES = ROOT / "mag" / "homology_graph" / "edges.tsv.gz"
HOMOLOGY_NODES = ROOT / "mag" / "homology_graph" / "maps" / "nodes_extract.tsv"
...
def stage_split(*, seed: int = SEED) -> dict:
    """Run paralogs_only split-predict + materialize SPLIT + LegNet TSV (CPU)."""
    ...
    _require(HOMOLOGY_EDGES, "file")
    _require(HOMOLOGY_NODES, "file")
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_SPLIT,
        resources="paralogs_only C++ + materialize SPLIT + LegNet TSV",
        log=str(ROOT / "logs" / f"{RUN_NAME}_split.log"),
    )

    print(
        f"split_predict type=paralogs_only edges={HOMOLOGY_EDGES} "
        f"nodes={HOMOLOGY_NODES} seed={seed}",
        flush=True,
    )
    ...
    print(f"paralogs_only counts: {counts}", flush=True)
    ...
    manifest = {
        ...
        "model": MODEL,
        "split": SPLIT,
        "homology_edges": str(HOMOLOGY_EDGES),
        "homology_nodes": str(HOMOLOGY_NODES),
        "split_counts": counts,
        "ratios_note": (
            "Direct: paralogs_only (1-rep/OG→train; remainder≈50/50). "
