"""Continue run31: reuse run27 clusters → 5-fold LOO LegNet + adversarial.

Source (read-only):
  ``runs_unif/legnet/run27_legnet_pangenome_k7_w0_100``
  (pangenome k=7, window 0…100 graph/assignment)

New:
  ``src/runs_unif/run31_legnet_pangenome_k7_w0_100_loo5``
  ``runs_unif/legnet/run31_legnet_pangenome_k7_w0_100_loo5``

Launch::

  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run31_legnet_pangenome_k7_w0_100_loo5.continue_from_split
"""
...
def stage_split() -> dict:
    ...
    assign = _require(SOURCE_UNIF / "pangenome_assignment.csv", "file")
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_STAGE,
        resources=f"reuse run27 clusters → LOO n_cv={N_CV} materialize",
