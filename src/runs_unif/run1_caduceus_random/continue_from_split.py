"""Continue legacy run1 with rebuilt ≈3:1:1 split → Caduceus direct + adversarial.

Legacy (read-only):
  ``src/runs/run1``, ``runs/run1`` (Caduceus / random / ready_caduceus)

New (write):
  ``src/runs_unif/run1_caduceus_random``
  ``runs_unif/caduceus/run1_caduceus_random``

Flow:
  1. Rewrite ``split.csv`` from the present table to train:test:val ≈ 3:1:1
     (legacy inverted 1:1:3 → swap train↔val; never mutate ``runs/run1``).
  2. Materialize ``SPLIT/`` (CPU; no GPU wait).
...
# Aligned with legacy run1 Caduceus train defaults (1 GPU).
BATCH_SIZE = 192
MAX_LENGTH = 256
NUM_WORKERS = 4
...
def stage_split(*, seed: int = SEED) -> dict:
    """Rewrite split table + materialize SPLIT (CPU only)."""
    ...
    rewrite_info = rewrite_split_table_aligned(
        ...
        allow_id_reassign=True,  # random split: ID reassign OK if swap fails
    )
