"""Continue legacy run17_pangenome_CDS_legnet with rebuilt ≈3:1:1 split → LegNet direct.

Legacy (read-only):
  ``src/runs/run17_pangenome_CDS_legnet``, ``runs/run17_pangenome_CDS_legnet``
  (pangenome contingency k=21, CDS window 0..100)

New (write):
  ``src/runs_unif/run17_legnet_pangenome_k21_w0_100``
  ``runs_unif/legnet/run17_legnet_pangenome_k21_w0_100``

Flow:
  1. Rewrite ``split.csv`` from the present SBS/pangenome assignment to
     train:test:val ≈ 3:1:1 (**whole clusters**; never mutate legacy).
  2. Stage pangenome intermediates + materialize SPLIT + LegNet TSV.
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run17_legnet_pangenome_k21_w0_100.continue_from_split
"""
...
MODEL = "legnet"
SPLIT = "pangenome"
SPLIT_PARAMS = "k21_w0_100"
RUN_NAME = f"run{RUN_I}_{MODEL}_{SPLIT}_{SPLIT_PARAMS}"

LEGACY_SRC = ROOT / "src" / "runs" / "run17_pangenome_CDS_legnet"
LEGACY_OUT = ROOT / "runs" / "run17_pangenome_CDS_legnet"
...
# Pangenome intermediates to stage from legacy (never mutate source).
PANGENOME_INTERMEDIATES = (
    "sbs_assignment.csv",
    "pangenome_assignment.csv",
    "pangenome_split_meta.json",
    "pangenome_graph_plot_meta.json",
)
...
def stage_split(*, seed: int = SEED) -> dict:
    """Rewrite split table + stage pangenome sidecars + materialize SPLIT + TSV."""
    ...
    # Homology/pangenome: reassign whole clusters → ≈3:1:1; never ID-level shuffle.
...
def run_train_direct(
    *,
    ...
    append_queue_entry(
        ...
        gpus=(gpu,),
        resources=(
            f"batch {batch}; direct {epochs}/{min_epochs}/p{patience}; no adversarial"
        ),
