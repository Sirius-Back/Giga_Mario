"""Run43: VGAE Stage-1 lossfix → LegNet direct + adversarial.

New (write):
  ``src/runs_unif/run43_legnet_vgae_stage1_k5_lossfix``
  ``runs_unif/legnet/run43_legnet_vgae_stage1_k5_lossfix``

Flow:
  1. GCN/VGAE cascade on pretrained ``VGAE/stage1_region_k5_lossfix`` (homology_first, k=5):
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run43_legnet_vgae_stage1_k5_lossfix.continue_from_split
"""
...
def stage_split(*, seed: int = SEED, gcn_model: str = GCN_MODEL) -> dict:
    ...
    from src.runs_unif.run43_legnet_vgae_stage1_k5_lossfix.ensure_mice_fold import (
        main as ensure_fold,
    )
