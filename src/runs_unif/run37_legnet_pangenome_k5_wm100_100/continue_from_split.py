"""Continue run37: 1-GPU LegNet direct + adversarial (after split_done).

Reuse MARKED (−100..100); pangenome k=5 + modularity.

Expects ``runs_unif/legnet/run37_legnet_pangenome_k5_wm100_100/{split.csv,SPLIT,legnet_input}``.
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run37_legnet_pangenome_k5_wm100_100.continue_from_split
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_done.is_file():
        from src.runs_unif.run37_legnet_pangenome_k5_wm100_100.run_split_cpu import (
            main as split_main,
        )
