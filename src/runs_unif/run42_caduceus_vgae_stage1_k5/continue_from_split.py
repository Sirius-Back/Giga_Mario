"""Run42: reuse run41 VGAE Stage-1 split → Caduceus direct + adversarial.

Source (read-only):
  ``runs_unif/legnet/run41_legnet_vgae_stage1_k5``
  (VGAE stage1_region_k5 reuse + mice ZSV overlay)

New (write):
  ``src/runs_unif/run42_caduceus_vgae_stage1_k5``
  ``runs_unif/caduceus/run42_caduceus_vgae_stage1_k5``

Flow:
  1. Wait until run41 ``split_done.json`` + ``split.csv`` exist.
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs_unif.run42_caduceus_vgae_stage1_k5.continue_from_split
"""
...
MODEL = "caduceus"
SPLIT = "vgae"
SPLIT_PARAMS = "stage1_k5"
RUN_NAME = f"run{RUN_I}_{MODEL}_{SPLIT}_{SPLIT_PARAMS}"

SOURCE_UNIF = ROOT / "runs_unif" / "legnet" / "run41_legnet_vgae_stage1_k5"
...
def wait_for_source_split(*, poll_sec: int = SOURCE_POLL_SEC) -> Path:
    """Block until run41 has a reusable VGAE split."""
...
def stage_split(*, seed: int = SEED) -> dict:
    """Reuse run41 VGAE split table + materialize Caduceus SPLIT (CPU only)."""
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_SPLIT,
        resources="reuse run41 VGAE stage1 split + materialize ready_caduceus",
    ...
    rewrite_info = {
        "method": "reuse_run41_unif_vgae_stage1_split_csv",
        ...
        "ratios": list(RATIOS),
        "note": (
            "Run41 reused VGAE/stage1_region_k5 labeling + mice ZSV overlay; "
