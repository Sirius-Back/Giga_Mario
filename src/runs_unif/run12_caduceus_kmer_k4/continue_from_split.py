"""Continue legacy run12_4mer_caduceus with rebuilt ≈3:1:1 split → Caduceus direct only.

Legacy (read-only):
  ``src/runs/run12_4mer_caduceus``, ``runs/run12_4mer_caduceus``
  (Caduceus / kmer k=4 / ready_caduceus)

New (write):
  ``src/runs_unif/run12_caduceus_kmer_k4``
  ``runs_unif/caduceus/run12_caduceus_kmer_k4``
...
LEGACY_SRC = ROOT / "src" / "runs" / "run12_4mer_caduceus"
LEGACY_OUT = ROOT / "runs" / "run12_4mer_caduceus"
