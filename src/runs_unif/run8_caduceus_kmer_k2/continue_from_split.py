"""Continue legacy run8_2mer_caduceus with rebuilt ≈3:1:1 split → Caduceus direct only.

Legacy (read-only):
  ``src/runs/run8_2mer_caduceus``, ``runs/run8_2mer_caduceus``
  (Caduceus / kmer k=2 / ready_caduceus)

New (write):
  ``src/runs_unif/run8_caduceus_kmer_k2``
  ``runs_unif/caduceus/run8_caduceus_kmer_k2``
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs_unif.run8_caduceus_kmer_k2.continue_from_split
"""
...
LEGACY_SRC = ROOT / "src" / "runs" / "run8_2mer_caduceus"
LEGACY_OUT = ROOT / "runs" / "run8_2mer_caduceus"
