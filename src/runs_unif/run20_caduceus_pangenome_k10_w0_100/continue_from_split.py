"""Continue unif run19 pangenome split → Caduceus direct + adversarial.

Source (read-only):
  ``runs_unif/legnet/run19_legnet_pangenome_k10_w0_100``
  (pangenome k=10, window 0…100, modularity-refined CCs, ≈3:1:1)

New (write):
  ``src/runs_unif/run20_caduceus_pangenome_k10_w0_100``
  ``runs_unif/caduceus/run20_caduceus_pangenome_k10_w0_100``

Flow:
  1. Reuse run19 ``split.csv`` (+ assignment / graph sidecars). Verify ≈3:1:1
     and fold sizes (modularity already applied; re-check max_fold_size).
...
MODEL = "caduceus"
SPLIT = "pangenome"
SPLIT_PARAMS = "k10_w0_100"
RUN_NAME = f"run{RUN_I}_{MODEL}_{SPLIT}_{SPLIT_PARAMS}"

SOURCE_UNIF = ROOT / "runs_unif" / "legnet" / "run19_legnet_pangenome_k10_w0_100"
...
MAX_FOLD_SIZE = 1000  # modularity cap from run19
...
def stage_split(*, seed: int = SEED, max_fold_size: int = MAX_FOLD_SIZE) -> dict:
    """Reuse run19 split table + materialize Caduceus SPLIT (CPU only)."""
    ...
    # Ensure split.csv columns normalized; keep run19 labels (already ≈3:1:1).
    ...
    rewrite_info = {
        "method": "reuse_run19_unif_split_csv",
        ...
        "ratios": list(RATIOS),
        "note": (
            "Run19 already applied C++ contingency k=10 + modularity refine "
            "for large connected components; folds reused for Caduceus panel."
        ),
    }
