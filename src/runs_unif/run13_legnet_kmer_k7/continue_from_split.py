"""Continue legacy run13_7mer_legnet with rebuilt 3:1:1 split → LegNet direct only (no adversarial).

Legacy (read-only):
  ``src/runs/run13_7mer_legnet``, ``runs/run13_7mer_legnet`` (kmer k=7 / cpp)

New (write):
  ``src/runs_unif/run13_legnet_kmer_k7``
  ``runs_unif/legnet/run13_legnet_kmer_k7``

Flow:
  1. Rewrite ``split.csv`` from the present table to train:test:val ≈ 3:1:1
     (prefer train↔val swap when legacy was inverted; never mutate ``runs/run13_7mer_legnet``).
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run13_legnet_kmer_k7.continue_from_split
"""
...
LEGACY_OUT = ROOT / "runs" / "run13_7mer_legnet"
...
def stage_split(*, seed: int = SEED) -> dict:
    ...
    for rel in KMER_INTERMEDIATES:
        ...
        dest = OUT_ROOT / rel
        if dest.exists():
            # Already written by cluster→train/test/val rewrite (sbs_assignment).
    ...
    manifest = {
        ...
        "run_name": RUN_NAME,
        "legacy": {
            "src": str(ROOT / "src" / "runs" / "run13_7mer_legnet"),
            "out": str(LEGACY_OUT),
        },
