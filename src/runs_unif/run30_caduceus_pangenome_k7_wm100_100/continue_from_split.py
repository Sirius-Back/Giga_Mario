"""Continue unif run29 pangenome split → Caduceus direct + adversarial.

Source (read-only):
  ``runs_unif/legnet/run29_legnet_pangenome_k7_wm100_100``
  (pangenome k=7, panel MARKED −100…100, modularity-refined CCs, ≈3:1:1)

New (write):
  ``src/runs_unif/run30_caduceus_pangenome_k7_wm100_100``
  ``runs_unif/caduceus/run30_caduceus_pangenome_k7_wm100_100``

Flow:
  1. Wait until run29 ``split.csv`` (+ sidecars / ``split_done.json``) exist.
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs_unif.run30_caduceus_pangenome_k7_wm100_100.continue_from_split
"""
...
MAX_FOLD_SIZE = 1000  # modularity cap from run29
...
def wait_for_source_split(*, poll_sec: int = SOURCE_POLL_SEC) -> Path:
    """Block until run29 has a reusable split table (prefer split_done.json)."""
...
def stage_split(*, seed: int = SEED, max_fold_size: int = MAX_FOLD_SIZE) -> dict:
    """Reuse run29 split table + materialize Caduceus SPLIT (CPU only)."""
    ...
    # Ensure split.csv columns normalized; keep run29 labels (already ≈3:1:1).
    ...
    rewrite_info = {
        "method": "reuse_run29_unif_split_csv",
        ...
        "ratios": list(RATIOS),
        "note": (
            "Run29 applied pangenome k=7 on panel MARKED (−100..100) + modularity refine for large "
