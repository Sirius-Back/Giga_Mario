"""CPU stages for run19: reuse run17 MARKED_pangenome + new k=10 C++ graph.
...
- Materialize SPLIT + LegNet TSV under ``runs_unif/legnet/run19_legnet_pangenome_k10_w0_100``

Launch::

  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run19_legnet_pangenome_k10_w0_100.run_split_cpu
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.runs_unif.run19_legnet_pangenome_k10_w0_100.ensure_mice_fold import (
        main as ensure_fold,
    )
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB,
        resources="reuse run17 MARKED; new C++ contingency k=10 + modularity + plot",
