"""Continue run29: 1-GPU LegNet direct + adversarial (after split_done).

Panel MARKED (−100..100) + pangenome k=7. Expects
``runs_unif/legnet/run29_legnet_pangenome_k7_wm100_100/{split.csv,SPLIT,legnet_input}``.
Waits for **1** free GPU unless ``skip_wait``. Runs CPU split first if missing
(so GPU wait happens after split reuse / build).
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run29_legnet_pangenome_k7_wm100_100.continue_from_split
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not split_done.is_file():
        from src.runs_unif.run29_legnet_pangenome_k7_wm100_100.run_split_cpu import (
            main as split_main,
        )
