"""CPU stages for run29: reuse panel MARKED (−100..100) + new k=7 C++ graph.

- Use ``ready_legnet/MARKED`` as ``MARKED_pangenome`` (``reuse_panel_marked=True``;
  LegNet CRS / gene window −100..100 — no re-adapt from raw)
...
- Materialize SPLIT + LegNet TSV under ``runs_unif/legnet/run29_legnet_pangenome_k7_wm100_100``

Launch::

  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run29_legnet_pangenome_k7_wm100_100.run_split_cpu
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.runs_unif.run29_legnet_pangenome_k7_wm100_100.ensure_mice_fold import (
        main as ensure_fold,
    )
    ...
    meta = {
        ...
        "marked_fasta": str(PANEL_ROOT / "MARKED"),
        "reuse_panel_marked": True,
        "note": "Use ready_legnet/MARKED as MARKED_pangenome (window −100..100); no re-adapt",
    }
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB,
        resources="reuse panel MARKED (−100..100); C++ contingency k=7 + modularity + plot",
