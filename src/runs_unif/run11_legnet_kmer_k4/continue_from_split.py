"""Continue legacy run11_4mer_legnet with rebuilt 3:1:1 split → LegNet direct only (no adversarial).

Legacy (read-only):
  ``src/runs/run11_4mer_legnet``, ``runs/run11_4mer_legnet`` (kmer k=4 / cpp)

New (write):
  ``src/runs_unif/run11_legnet_kmer_k4``
  ``runs_unif/legnet/run11_legnet_kmer_k4``

Flow:
  1. Rewrite ``split.csv`` from the present table to train:test:val ≈ 3:1:1
     (prefer train↔val swap when legacy was inverted; never mutate ``runs/run11_4mer_legnet``).
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run11_legnet_kmer_k4.continue_from_split
"""
...
LEGACY_OUT = ROOT / "runs" / "run11_4mer_legnet"
...
def stage_split(*, seed: int = SEED) -> dict:
    ...
    manifest = {
        ...
        "run_name": RUN_NAME,
        "legacy": {
            "src": str(ROOT / "src" / "runs" / "run11_4mer_legnet"),
            "out": str(LEGACY_OUT),
        },
