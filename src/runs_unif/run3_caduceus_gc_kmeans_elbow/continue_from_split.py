"""Continue legacy run3 with rebuilt ≈3:1:1 split → Caduceus direct only.

Legacy (read-only):
  ``src/runs/run3``, ``runs/run3`` (Caduceus / GC / kmeans_elbow / ready_caduceus)

New (write):
  ``src/runs_unif/run3_caduceus_gc_kmeans_elbow``
  ``runs_unif/caduceus/run3_caduceus_gc_kmeans_elbow``

Flow:
  1. Rewrite ``split.csv`` from the present SBS/GC assignment to
     train:test:val ≈ 3:1:1 (**whole clusters**; never mutate legacy).
  2. Stage GC intermediates + materialize SPLIT.
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs_unif.run3_caduceus_gc_kmeans_elbow.continue_from_split
"""
...
LEGACY_SRC = ROOT / "src" / "runs" / f"run{RUN_I}"
...
# Aligned with legacy run3 Caduceus train defaults (1 GPU).
BATCH_SIZE = 480
MAX_LENGTH = 208
NUM_WORKERS = 4
...
def stage_split(*, seed: int = SEED) -> dict:
    """Rewrite split table + stage GC sidecars + materialize SPLIT."""
    ...
    rewrite_info = rewrite_split_table_aligned(
        ...
        allow_id_reassign=False,  # GC SBS: never random ID sweep
    )
