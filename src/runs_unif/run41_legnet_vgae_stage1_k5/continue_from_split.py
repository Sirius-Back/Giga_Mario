"""Run41: VGAE Stage-1 split → LegNet direct + adversarial.

New (write):
  ``src/runs_unif/run41_legnet_vgae_stage1_k5``
  ``runs_unif/legnet/run41_legnet_vgae_stage1_k5``

Flow:
  1. GCN/VGAE cascade on pretrained ``VGAE/stage1_region_k5``:
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs_unif.run41_legnet_vgae_stage1_k5.continue_from_split
"""
...
def stage_split(*, seed: int = SEED, gcn_model: str = GCN_MODEL) -> dict:
    ...
    from src.runs_unif.run41_legnet_vgae_stage1_k5.ensure_mice_fold import (
        main as ensure_fold,
    )
