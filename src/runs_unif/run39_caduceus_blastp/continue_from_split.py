"""Continue legacy run15 blastp split → Caduceus direct + adversarial.

Legacy (read-only):
  ``src/runs/run15_blastp_legnet``, ``runs/run15_blastp_legnet``
  (BLASTP CDS homology CC / ready_legnet panel; inverted ≈1:1:3)

New (write):
  ``src/runs_unif/run39_caduceus_blastp``
  ``runs_unif/caduceus/run39_caduceus_blastp``

Flow:
  1. Rewrite ``split.csv`` to train:test:val ≈ 3:1:1 at **cluster grain**
     (``sbs_assignment`` / blastp CC; never mutate ``runs/run15_blastp_legnet``).
     Label swap alone cannot fix ≈1:1:3 → ~6:1:1 after train↔val.
  2. Stage blastp sidecars + materialize ``SPLIT/`` from ``ready_caduceus``.
  3. Wait until **1** GPU is free, then direct Caduceus
     (min 15 / max 30 / patience 10) + mice ZSV.
  4. Adversarial: random ⊆ direct IDs + fold-class; max 10 / patience 5 + ZSV.
  5. Reports / TensorBoard via pipeline viz + train_monitor.

Launch::

  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs_unif.run39_caduceus_blastp.continue_from_split
"""
...
LEGACY_SRC = ROOT / "src" / "runs" / "run15_blastp_legnet"
LEGACY_OUT = ROOT / "runs" / "run15_blastp_legnet"
...
BLASTP_INTERMEDIATES = (
    "sbs_assignment.csv",
    "blastp_assignment.csv",
    "blastp_split_meta.json",
    "split_cpu_meta.json",
    "protein_mapping.json",
)
...
def stage_split(*, seed: int = SEED) -> dict:
    """Rewrite blastp clusters to ≈3:1:1 + materialize Caduceus SPLIT (CPU)."""
    ...
    append_queue_entry(
        ...
        job_class=CLASS_CPU_RAM_HEAVY,
        peak_ram_gib=PEAK_RAM_GIB_SPLIT,
        log=str(ROOT / "logs" / f"{RUN_NAME}_pipeline.log"),
    )

    # Cluster-grain redo (swap alone cannot turn ≈1:1:3 into ≈3:1:1).
    rewrite_info = rewrite_split_table_aligned(
        ...
        allow_id_reassign=False,  # blastp CC: never random ID sweep
    )
    print(f"split rewrite: {json.dumps(rewrite_info, sort_keys=True)}", flush=True)

    staged_extra: dict[str, str] = {}
    for rel in BLASTP_INTERMEDIATES:
        src = LEGACY_OUT / rel
        if not src.is_file():
            continue
        dest = OUT_ROOT / rel
        if dest.exists():
            # Prefer rewritten sbs_assignment.csv from rewrite_split_table_aligned.
        ...
        json.dumps(
            {
                ...
                "rewrite": rewrite_info.get("method"),
            },
            indent=2,
        )
        + "\n",
