#!/usr/bin/env python3
"""Fine-tune Caduceus for continuous TPM regression on @adapt caduceus_ready folds.

Multi-GPU via torchrun. Logs metrics.md suite each epoch under
runs/.../epoch{N}/metrics.json (+ metrics.log). Saves final_model/ and train_time.json.

Epoch directories are 1-indexed (epoch1..epochN) per caduceus-full deliverables.
"""
...
class TpmWindowDataset(Dataset):
    ...
    def __init__(self, data_root: Path, split: str, tokenizer, max_length: int):
        ...
        with labels_path.open(encoding="utf-8") as fh:
            ...
            for line in fh:
                ...
                self.samples.append((seq_path, tpm))
        if not self.samples:
    ...
    def __getitem__(self, idx: int):
        ...
        item["labels"] = torch.tensor(tpm, dtype=torch.float32)
        return item
