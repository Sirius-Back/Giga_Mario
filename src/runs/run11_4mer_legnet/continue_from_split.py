"""Continue run11_4mer_legnet from existing 4-mer split → LegNet + adversarial.

Assumes ``runs/run11_4mer_legnet/{split.csv,SPLIT,legnet_input/all.tsv}`` and mice
ZSV trees already exist.

Launch (after GPUs 2,3 free)::

  CUDA_VISIBLE_DEVICES=2,3 conda run -n legnet --no-capture-output \\
    python -m src.runs.run11_4mer_legnet.continue_from_split
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"run11_4mer_legnet continue COMPLETED → {OUT_ROOT}", flush=True)
    return 0
