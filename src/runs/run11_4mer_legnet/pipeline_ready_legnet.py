"""run11_4mer_legnet orchestrator: CPU 4-mer split → wait GPUs 2,3 → train.

- Split: k-mer k=4, engine=cpp, cluster=kmeans_elbow, mice ZSV
...
Launch::
  ...
  conda run -n legnet --no-capture-output \\
    python -m src.runs.run11_4mer_legnet.pipeline_ready_legnet
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not skip_split and not (OUT_ROOT / "split_cpu_done.json").is_file():
        from src.runs.run11_4mer_legnet.run_split_cpu import main as split_main
    ...
    from src.runs.run11_4mer_legnet.continue_from_split import main as train_main
