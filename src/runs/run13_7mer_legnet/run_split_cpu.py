"""CPU-only stages for run13_7mer_legnet: mice fold + 7-mer SBS split + LegNet TSV.

No GPU train. Writes under ``runs/run13_7mer_legnet/``.

Launch::

  conda run -n legnet --no-capture-output \\
    python -m src.runs.run13_7mer_legnet.run_split_cpu
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.runs.run13_7mer_legnet.ensure_mice_fold import main as ensure_fold
    ...
    print(f"run13 split_cpu meta={meta}", flush=True)
    ...
    print(f"run13 split_cpu COMPLETED → {OUT_ROOT}", flush=True)
