"""Continue run15 from existing BLASTP ``split.csv`` → LegNet direct train.

Skips adapt / BLASTP. Expects ``runs/run15_blastp_legnet/{split.csv,SPLIT,legnet_input/all.tsv}``.
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"run15_blastp_legnet continue COMPLETED → {OUT_ROOT}", flush=True)
    return 0
