"""CPU-only: mice fold + 4-mer SBS (cpp) + materialize SPLIT for Caduceus.
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs.run12_4mer_caduceus.run_split_cpu
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.runs.run12_4mer_caduceus.ensure_mice_fold import main as ensure_fold
    ...
    print(f"run12 split_cpu meta={meta}", flush=True)
    ...
    print(f"run12 split_cpu COMPLETED → {OUT_ROOT}", flush=True)
