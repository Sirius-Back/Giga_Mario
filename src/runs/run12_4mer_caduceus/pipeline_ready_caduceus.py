"""run12_4mer_caduceus: CPU 4-mer split → wait GPUs 0,1 → Caduceus + adversarial.

- Split: k-mer k=4, engine=cpp, cluster=kmeans (fixed n_clusters=8, fastest)
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs.run12_4mer_caduceus.pipeline_ready_caduceus
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not skip_split and not (OUT_ROOT / "split_cpu_done.json").is_file():
        from src.runs.run12_4mer_caduceus.run_split_cpu import main as split_main
    ...
    from src.runs.run12_4mer_caduceus.continue_from_split import main as train_main
