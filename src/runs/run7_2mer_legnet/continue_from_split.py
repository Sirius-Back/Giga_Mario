"""Continue run7_2mer_legnet from existing 2-mer split → LegNet + adversarial.

Assumes ``runs/run7_2mer_legnet/{split.csv,SPLIT,legnet_input/all.tsv}`` and mice
ZSV trees already exist. Skips k-mer feature recompute.

Launch::

  CUDA_VISIBLE_DEVICES=2,3 conda run -n legnet --no-capture-output \\
    python -m src.runs.run7_2mer_legnet.continue_from_split
"""
...
OUT_ROOT = ROOT / "runs" / "run7_2mer_legnet"
EPOCHS = 50
MIN_EPOCHS = 10
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"run7_2mer_legnet continue complete → {OUT_ROOT}", flush=True)
    return 0
