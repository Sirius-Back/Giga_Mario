"""run14_7mer_caduceus: CPU 7-mer split → wait GPUs 0,1 → Caduceus + adversarial.

- Split: k-mer k=7, engine=cpp, cluster=kmeans (fixed n_clusters=8, fastest)
...
Launch::
  ...
  conda run -n caduceus_env --no-capture-output \\
    python -m src.runs.run14_7mer_caduceus.pipeline_ready_caduceus
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    from src.runs.run14_7mer_caduceus.continue_from_split import main as train_main
