"""Continue run14_7mer_caduceus from 7-mer split → Caduceus + adversarial.

Assumes ``runs/run14_7mer_caduceus/{split.csv,SPLIT}`` + mice ZSV trees exist.

Launch (after GPUs 0,1 free)::

  CUDA_VISIBLE_DEVICES=0,1 conda run -n caduceus_env --no-capture-output \\
    python -m src.runs.run14_7mer_caduceus.continue_from_split
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"continue_from_split run_id={RUN_ID} n_devices={n_devices} "
    ...
    try:
        ...
        print(f"WARNING: direct pipeline_viz skipped: {type(exc).__name__}: {exc}", flush=True)
