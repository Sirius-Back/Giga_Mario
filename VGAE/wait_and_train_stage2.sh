...
LOG=VGAE/stage2_hash_k5/train.log
mkdir -p VGAE/stage2_hash_k5

# Wait for Stage1 completion
while [ ! -f VGAE/stage1_region_k5/train_meta.json ]; do
  echo "=== $(date -Iseconds) waiting for Stage1 train_meta.json ===" | tee -a "$LOG"
  sleep 600
done
echo "Stage1 complete; starting Stage2 hash VGAE" | tee -a "$LOG"

while true; do
  echo "=== $(date -Iseconds) polling for free GPU (stage2) ===" | tee -a "$LOG"
...
for g in range(torch.cuda.device_count()):
    ok, _ = can_launch_parallel(peak_ram_gib=16.0, gpus=(g,), job_class=CLASS_GPU_TRAIN)
    if not ok:
        continue
    try:
...
PY
  ...
  if [ -n "$DEV" ]; then
    echo "Launching Stage2 VGAE on $DEV" | tee -a "$LOG"
